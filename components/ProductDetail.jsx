...
const ProductDetail = ({ game, prefilledOption = null, allGames, onBack, onBuyNow, onGameClick, promoPrice }) => {
  ...
  const availableAccounts = React.useMemo(() => {
    ...
    // Legacy Fallback (If PS4/PS5 prices aren't set in Admin yet)
    if (accounts.length === 0 && game.price) {
      accounts.push({ id: 'Activated Account', label: 'Activated', price: game.discount_price || game.price, original: game.price, stock: game.activated_stock, promo: promoPrice?.activated });
      if (game.deactivated_price) {
        accounts.push({ id: 'Deactivated Account', label: 'Deactivated', price: game.deactivated_discount || game.deactivated_price, original: game.deactivated_price, stock: game.deactivated_stock, promo: promoPrice?.deactivated });
      }
    }
    return accounts;
  ...
  useEffect(() => {
    ...
    if (!isGiftCard && availableAccounts.length > 0) {
      // Auto-select the first in-stock account, or just the first one if all are out of stock
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-[#121212] pb-32 animate-in fade-in duration-300 transition-colors">
          ...
          <div className="flex flex-col mb-8 border-b border-gray-100 dark:border-gray-800 pb-8">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {game.options?.map((opt, idx) => (
                <button 
                  ...
                  className={`py-4 px-3 rounded-2xl border-2 text-center transition-all ${selectedOption?.label === opt.label ? 'border-black dark:border-white bg-white dark:bg-gray-800 shadow-sm ring-1 ring-black/5 dark:ring-white/5' : 'border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900 hover:border-gray-300 dark:hover:border-gray-700'}`}
                >
                  <p className={`text-sm font-black ${selectedOption?.label === opt.label ? 'text-black dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}>{opt.label}</p>
                </button>
              ))}
            </div>
            ...
            <div className="flex justify-center">
              <div className="flex items-center gap-6 bg-gray-50 dark:bg-[#0a0a0a] rounded-full px-5 py-2.5 border border-gray-200 dark:border-gray-800 shadow-inner">
                ...
                <button onClick={() => setQuantity(q => q + 1)} className="w-8 h-8 flex items-center justify-center rounded-full bg-black dark:bg-white text-white dark:text-black shadow-md font-bold text-xl transition-transform active:scale-95">+</button>
              </div>
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-3 mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {availableAccounts.map(acc => {
                ...
                const hasDiscount = acc.promo ? true : (acc.price < acc.original);
                const displayPrice = acc.promo || acc.price;
                const displayOriginal = acc.promo ? acc.price : acc.original;
                ...
                return (
                  <button 
                    ...
                    disabled={isOut}
                    className={`p-3 rounded-xl border-2 text-left transition-all relative overflow-hidden flex flex-col ${isSelected ? 'border-black dark:border-white bg-black dark:bg-white text-white dark:text-black shadow-md' : 'border-gray-200 dark:border-gray-800 bg-white dark:bg-[#121212] text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-700'} ${isOut ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    <p className="text-[10px] font-bold mb-1 opacity-80 uppercase tracking-wide">{acc.label}</p>
                    <div className="flex flex-col">
                      <p className={`text-sm font-black ${isSelected ? 'text-white dark:text-black' : (hasDiscount && acc.promo ? 'text-gray-900 dark:text-white' : 'text-black dark:text-white')}`}>
                        {displayPrice.toLocaleString()} MMK
                      </p>
                      {hasDiscount && (
                        <p className={`text-[10px] font-bold line-through ${isSelected ? 'text-white/70 dark:text-black/70' : 'text-gray-400 dark:text-gray-500'}`}>
                          {displayOriginal.toLocaleString()} MMK
                        </p>
                      )}
                    </div>
                    <div className={`mt-2 text-[8px] font-black px-2 py-1 rounded inline-block uppercase tracking-wider ${!isOut ? (isSelected ? 'bg-white/20 dark:bg-black/20 text-white dark:text-black' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400') : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'}`}>
        ...
        {!isGiftCard && game.youtube_link && (
          <div className="mb-6 border-t border-gray-100 dark:border-gray-800 pt-6">
            ...
            {!isPlayingTrailer && youtubeId ? (
              <button onClick={() => setIsPlayingTrailer(true)} className="w-full flex items-center justify-center gap-2 bg-gray-100 dark:bg-gray-800 text-black dark:text-white py-3 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-gray-700 active:scale-95 transition-all">
            ...
            {recommendedGames.map(rGame => (
              <div key={rGame.id} onClick={() => onGameClick(rGame)} className="min-w-[130px] max-w-[130px] snap-start flex flex-col gap-2 cursor-pointer active:scale-95 transition-transform">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-black dark:text-gray-300 mt-0.5">{(rGame.discount_price || rGame.price).toLocaleString()} MMK</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white dark:bg-[#121212] border-t border-gray-100 dark:border-gray-800 p-4 flex flex-col gap-3 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.03)] dark:shadow-[0_-4px_20px_rgba(0,0,0,0.4)]">
